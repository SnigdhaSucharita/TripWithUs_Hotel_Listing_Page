...
app.get("/", (req, res) => {
  res.json("Welcome to TripWithUs");
})

function sortHotelsByPriceDescending(hotel1, hotel2) {
  return hotel2.price - hotel1.price;
}

function sortHotelsByPriceAscending(hotel1, hotel2) {
...
app.get("/hotels/sort/pricing", (req, res) => {
  ...
  if(pricing.toLowerCase() === "high-to-low") {
    let result = hotelsCopy.sort(sortHotelsByPriceDescending);
    res.json({ hotels: result });
  } else {
    let result = hotelsCopy.sort(sortHotelsByPriceAscending);
    res.json({ hotels: result });
  }
})
...
app.get("/hotels/sort/rating", (req, res) => {
  ...
  let hotelsCopy = hotels.slice();
  if(rating.toLowerCase() === "high-to-low") {
    let result = hotelsCopy.sort((a, b) => b.rating - a.rating);
    res.json({ hotels: result });
  } else {
    let result = hotelsCopy.sort((a, b) => a.rating - b.rating);
    res.json({ hotels: result });
  }
})
...
app.get("/hotels/sort/reviews", (req, res) => {
  ...
  if(reviews.toLowerCase() === "least-to-most") {
    let result = hotelsCopy.sort((a, b) => a.reviews - b.reviews);
    res.json({ hotels: result });
  } else {
    let result = hotelsCopy.sort((a, b) => b.reviews - a.reviews);
    res.json({ hotels: result });
  }
})
